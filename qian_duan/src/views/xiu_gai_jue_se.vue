    ...
    export default {
        ...
        methods:{
            tian_jia:function () {
                ...
                axios.get(`http://localhost:2333/xiu_gai_jue_se/${this.xu_hao}/${this.ming_cheng}/${this.zhuang_tai}`)
                    .then(response=>{
                        if(response.statusText === 'OK'){
                            alert('修改成功！！！')
                            _this.$router.push('/zhu_ye/jue_se_biao')
                        }
                    })
            }
        },
        mounted:function () {
            ...
            axios.get(`http://localhost:2333/xiu_gai_jue_se1/${this.$route.query.xu_hao}`)
                .then(response=>{
